...
fun SettingsScreen(
    ...
    var sbCs by rememberSaveable { mutableStateOf("") }
    var sbQueue by rememberSaveable { mutableStateOf("") }
    ...
    LaunchedEffect(Unit) {
        if (!fieldsReady) {
            ...
            xmlUrl = s.xmlUrl
            sbCs = s.serviceBusConnectionString
            sbQueue = s.serviceBusQueueName
    ...
    Scaffold(
        ...
        Column(
            ...
            SettingField(label = "Service bus connection string") {
                OutlinedTextField(
                    value = sbCs,
                    onValueChange = { sbCs = it; viewModel.updateServiceBusCs(it) },
                    modifier = Modifier.fillMaxWidth(),
                    minLines = 2
                )
            }

            SettingField(label = "Service bus queue name") {
                OutlinedTextField(
                    value = sbQueue,
                    onValueChange = { sbQueue = it; viewModel.updateServiceBusQueue(it) },
                    modifier = Modifier.fillMaxWidth(),
                    singleLine = true
                )
            }
